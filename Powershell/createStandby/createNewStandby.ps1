PARAM(
    ...
    [int] $skipRestore = 0,
    [int] $skipStndbyMGR = 0,
    [int] $overWriteTarget = 0,
    [int] $dryRun = 1
    #[string] $standbyMGR = ''
    ## These values should be table driven to try and eliminate typos....
    #  .\createNewStandby.ps1 -targetDB 'ExactTarget11106' -oldPROD 'ATL1S11C010I04\I04' -newStandBY 'ATL1S11CB010I04\I04' -overWriteTarget 1 -dryRun 0
...
function process-process ( [string] $f_sourceHost, [string] $f_targetHost, [string] $f_targetDB, [string] $f_targetBAK, [int] $f_skipRestore, [int] $f_skipStndbyMGR, [int] $f_overWriteTarget, [int] $f_dryRun )
{
            ...
            If( $f_skipRestore -eq 1)
                {
                    ...
                    WRITE-HOST `t"restoredatabase -RequestNum 711 -ServerInstance $f_targetHost -Database $f_targetDB -RestoreType "Database" -BackupFile $f_targetBAK -OverWriteDatabase "$f_overWriteTarget" -OperationType "Restore" -Verbose -RestoreLunPath "$newDataPath" -DryRun $f_skipRestore"
                    WRITE-HOST " "
                    $backupFile = restoredatabase -RequestNum 711 -ServerInstance $f_targetHost -Database $f_targetDB -RestoreType "Database" -BackupFile $f_targetBAK -OverWriteDatabase "$f_overWriteTarget" -OperationType "Restore" -Verbose -RestoreLunPath "$newDataPath" -DryRun $f_skipRestore
                    $backupPath = $backupFile.Replace("Disk = ","")
                }
            ELSE
                {
                    WRITE-HOST "[] Call Restore process - $f_targetHost "
                    WRITE-HOST `t"restoredatabase -RequestNum 711 -ServerInstance "$f_targetHost" -Database "$f_targetDB" -RestoreType "Database" -BackupFile "$f_targetBAK" -OverWriteDatabase "$f_overWriteTarget" -OperationType "Restore" -Verbose -RestoreLunPath "$newDataPath" -DryRun $f_dryRun"
                    WRITE-HOST " "
                    $backupFile = restoredatabase -RequestNum 711 -ServerInstance "$f_targetHost" -Database "$f_targetDB" -RestoreType "Database" -BackupFile "$f_targetBAK" -OverWriteDatabase "$f_overWriteTarget" -OperationType "Restore" -Verbose -RestoreLunPath "$newDataPath" -DryRun $f_dryRun
                    $backupPath = $backupFile.Replace("Disk = ","")
                }
        }
    CATCH
    ...
    $bak1Path = "\\"+ $f_targetHost.substring(0,$f_targetHost.IndexOf('\')) +"\"+ $instDrive.replace(':','$') +"\StandbyUtil"
    #enable LogcopyJob
    IF($f_skipStndbyMGR -eq 0)
            {
                $enableSQL = "EXEC msdb.dbo.sp_update_job @job_name=N'LogShip_"+ $f_targetDB +"', @enabled=1"

                IF( $f_targetHost -like '*S11*' ) 
                    {
                        $Instance
                        $logRestorePath = $instDrive +"\SQL\"+ $f_targetHost.substring($f_targetHost.IndexOf('\')+1,3) +"\Bak1\LogRestore"
                        $badRestorePath = $instDrive +"\SQL\"+ $f_targetHost.substring($f_targetHost.IndexOf('\')+1,3) +",10001\Bak1\LogRestore"
                        # $f_copyCMD = "robocopy $standbyUTIL $targetDrive /MIR /ETA"
                        $moveCMD = 'robocopy "'+ $badRestorePath +'" "'+ $($logRestorePath) +'" /MOVE /ETA'
                        $removeCMD =  "remove-item '$($badRestorePath.replace('\BAK1\LogRestore',''))' -recurse"
                        $updateSQL = "UPDATE StandbyDBManager.dbo.TargetInstance SET TargetCopyPath = '$($logRestorePath)' WHERE TargetCopyPath = '$($badRestorePath)'" 

                        $masterDSNPath = "\\"+ $f_targetHost.substring(0,$f_targetHost.IndexOf('\')) +"\"+ $instDrive.replace(':','$') +"\StandbyUtil_"+ $f_targetDB +"\bin\master-dsn.txt"
                        $connectionCMD = "get-content $($masterDSNPath)"
    ...
    If($f_dryRun -eq 1)
        {
            IF( $f_targetHost -like '*S11*' )
                {
                    WRITE-HOST "[DryRun] S11 bug move LogRestore - remove empty parent folder"
                    WRITE-HOST `t"$moveCMD"
                    WRITE-HOST `t"$removeCMD"
                    WRITE-HOST "[DryRun] S11 bug move update invetory"
                    WRITE-HOST `t"$updateSQL"  
                    WRITe-HOST "[DryRun] Get Connection String from master-DSN.txt"
                    WRITE-HOST `t"$connectionCMD"                     
                }
    ...
    ELSE
        {
            IF($f_skipStndbyMGR -eq 0)
            {
                IF( $f_targetHost -like '*S11*' )
                    {
                        if(!(Test-Path -Path $logRestorePath ))
                            { 
                                WRITE-HOST "[] Executing: $moveCMD " 
                                invoke-expression $moveCMD
                            }
                        ELSE
                            {
                                WRITE-HOST "LogRestore Folder Exists"
                            }

                        WRITE-HOST "[] Executing: $removeCMD "
                        invoke-expression $removeCMD
                        WRITE-HOST "[] Executing: $connectionCMD"
                        $connection = invoke-expression $connectionCMD

                        $targetINV = $connection.REPLACE('Data Source=','').REPLACE('.xt.local','').split('=', 1).substring(0, $connection.IndexOf(',')-21)
                        WRITE-HOST "[] Executing: $updateSQL "
                        invoke-sqlcmd -ServerInstance $targetINV -Query $updateSQL                     
                    }
...
WRITE-HOST "[] Over Write Target: $overWriteTarget"
...
ELSE
    {
        IF( !$backupPath  -AND $skipCopy -eq 0)
            {
        ...
        IF( !$backupPath -AND $skipCopy -eq 0)
            {
                ...
                ## GRABBING LAST FULL BACKUP
                move-LastFull $oldProd $newStandby $targetDB $newStndbyBAK1 $overWriteTarget $dryRun
            }
        # call funciton sourceHost newHost     dbName    locationBAK    skiprestore
        process-process $oldProd $newStandby $targetDB $newStndbyBAK1 $skipRestore $skipStndbyMGR $overWriteTarget $dryRun

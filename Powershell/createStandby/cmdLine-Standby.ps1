...
exec master.dbo.xp_cmdshell @cmd; -- running Standby DB Manager from command line" 
    ...
    $execCMD2 = "DECLARE @file varchar(1000) = ( SELECT ''"+ $instDrive +"\StandbyUtil_"+ $f_targetDB +"\Bin\StandbyManager_output.txt'' )
...
IF EXISTS ( SELECT * from #TEXTFILE_1 WHERE FIELD1 like ''%ERROR%''  )
	BEGIN
		...
		RAISERROR(''The StandbyManager reports an error - check "+ $instDrive +":\StandbyUtil_"+ $f_targetDB +"\Bin\StandbyManager_output.txt '', 20, 1,  @errorMessage) WITH LOG;
	END
"
...
switch ($sourceHost) 
    {
        ...
        {($_ -LIKE "atl1P*" )}  {$standbyMGR = 'XTGAP4DBA01.XT.LOCAL\I1,10001';      $standbyUTIL = '\\XTINP1DBA01\d$\Standbys\GA-P4-StandbyMgr\StandbyUtil';    break}
        {($_ -LIKE "atl1S*" )}  {$standbyMGR = 'ATL1S11DBA01I02.XT.LOCAL\I02,10001'; $standbyUTIL = '\\XTINP1DBA01\d$\Standbys\ATL-S11-StandbyMgr\StandbyUtil';  break}
...
Comments:
     ...
     .\cmdLine-Standby.ps1 -sourceHost ATL1S11C009I01\I01 -sourceDB ExactTarget11073 -targetHost ATL1S11CB009I01\I01 -backupFile 'E$\SQL\I01\BAK1\ExactTarget11073\ExactTarget11073_20190422_232017_a1.bak'
#######################################################################>
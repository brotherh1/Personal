...
(
    ...
    [Parameter(Mandatory=$true)] [string] $targetAction = '',	#Restore Database, Backup database Restore Log
...
WHERE 
command IN ('$targetAction') "

IF( $targetAction -eq "Restore Database" )
	{
		$backupProgressSQL = $backupProgressSQL + "
Go"
	}
ELSE
	{
		$backupProgressSQL = $backupProgressSQL + "AND DB_Name(database_id) like N'$targetDatabase%'
GO"
	}
...
function Get-LogSpaceUtilization
{
    ...
    $logSpaceUtilization = @(Invoke-Sqlcmd -ServerInstance $targetInstance -Database "master" -Query $logSpaceUtilizationSQL);
...
function Send-StatusEmail
{
    ...
    $emailBody = "$targetAction of $targetDatabase is in progress.`n`nSource Server: $targetInstance`n`n";
    ...
    $backupMessage = "$targetAction Progress: $databaseBackupProgress";
    ...
    $messageSubject = "$targetAction of $targetDatabase in progress"
...
function Send-LastEmail
{
    ...
    # Start the e-mail body
    $emailBody = "$targetAction of $targetDatabase has completed.`n`nSource Server: $targetInstance`n`n";
    $emailBody += "Disabling SQL Agent Job: HADR - MonitorBackup_$($targetDatabase.replace('ExactTarget','ET'))"

    $backupMessage = "$targetAction Completed.";
    ...
    $smtpFrom = "$TargetInstanceFrom@XT.LOCAL"
    $smtpTo = $emailRecipients;
    $messageSubject = "$targetAction of $targetDatabase has finished"

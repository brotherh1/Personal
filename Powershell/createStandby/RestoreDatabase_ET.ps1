...
{
    param( [string] $Databasenew, [string] $BackupPhysicalFileName, [string] $BackupTypeDescription, [string] $HasBackupChecksums )
   ...
   # $LogFileList_From_Backup.count
    ...
    { 
 <###################### 
 #      Below code is commented out. Since this applicable only for DEA certification 
        ...
        # "$DatFileNewFolderPath"
        # "$lunpath"
        # $datafilelist.PhysicalName        
...
<#####################
            try {
                ...
                {
                    Writeout -printmsg "The Log file [$DatFileNewFolderPath_new] does not exist or could not be verified. The new file name value is [$ValidFilename]"
                }
            ...
            for ( $i=0; $i -lt $BackupDBCnt; $i++)
            {
               ...
               # Writeout -printmsg "Starting Validation and 'Restore with NORecovery' process for the database [$database]"
                ...
                {    
                    ...
                    try{                               
                          $backupFile = ValidateAndRestore_Database -Databasenew $database -BackupPhysicalFileName $BackupListFullPath_str -BackupTypeDescription $RDHr.BackupTypeDescription -HasBackupChecksums $RDHr.HasBackupChecksums
...
History:  
     20182202 HBROTHERTON W-4693137 Cleaned up code and standardized the tabbing.

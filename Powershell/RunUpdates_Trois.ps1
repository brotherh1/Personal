...
##### Change Folder structures to navigate to the path where  ps1 files are stored, this is done to import modules without having to change the path every single time
$CurrentDirectory= split-path -parent $MyInvocation.MyCommand.Path
cd $CurrentDirectory

Function DeployUpdateStatsSQL{
param ([string]$TargetServerInstance,[string]$Database)
#### Check for inputs
if($TargetServerInstance -eq $null -or $TargetServerInstance -eq '') {
     [string] $argTargetServerInstance = read-host "Enter TargetServerInstance"
     Set-Variable -name TargetServerInstance -Value $argTargetServerInstance
    }
if($Database -eq $null -or $Database -eq '') {
     [string] $argTargetServerInstance = read-host "Enter DatabaseName"
     Set-Variable -name TargetServerInstance -Value $argTargetServerInstance
    }
Invoke-sqlcmd -Query "

If exists (select * from sys.tables where name='CommandLog')
...
Go
If  exists (select * from sys.procedures where name='CommandExecute')
Begin
...
BEGIN
  ...
  IF @LogToTable = 'Y'
  BEGIN
    INSERT INTO dbo.CommandLog (DatabaseName, SchemaName, ObjectName, ObjectType, IndexName, IndexType, StatisticsName, QueueID,PartitionNumber, ExtendedInfo,[rowcount], CommandType, Command)
    VALUES (@DatabaseName, @SchemaName, @ObjectName, @ObjectType, @IndexName, @IndexType, @StatisticsName,@QueueID, @PartitionNumber, @ExtendedInfo, @rowcount,@CommandType, @Command)
  END
...
If  exists (select * from sys.procedures where name='IndexOptimize')
Begin
...
@SmallObjectScanPercent	float = 100,
@SmallObjectRows		int   = 10000000,   -- 10 million
@LargeObjectScanPercent float = 1,
@LargeObjectrows    int   = 1000000000,  -- 1 billion 1,000,000,000
@ExtraLargeObjectRowScanPercent	float = 0.0038,
@HighPriTableAccessThreshold smallint=1,
@MediumPriTableAccessThreshold smallint = 30
...
BEGIN
  ...
  DECLARE @CurrentCommand15 nvarchar(max)
  DECLARE @CurrentCommand16 nvarchar(max)
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
		...
		set @CurrentCommand15=''
		set @CurrentCommand15='
		Select top 1 @paramcurrentobjectrowcount=rowcnt
		from ['+@CurrentDatabaseName+'].sys.sysindexes where id=@paramCurrentObjectID
		and indid in (1,0)'
		print(@CurrentCommand15)
		print (cast(@currentobjectid as varchar(max)))
		exec sp_executesql @statement=@currentcommand15, @params =N'@paramCurrentObjectID int, @paramcurrentobjectrowcount bigint output',@paramCurrentObjectID=@CurrentObjectID,@paramcurrentobjectrowcount=@currentobjectrowcount output

          EXECUTE @CurrentCommandOutput13 = [dbo].[CommandExecute] @Command = @CurrentCommand13, @CommandType = @CurrentCommandType13, @Mode = 2, @Comment = @CurrentComment, @DatabaseName = @CurrentDatabaseName, @SchemaName = @CurrentSchemaName, @ObjectName = @CurrentObjectName, @ObjectType = @CurrentObjectType, @IndexName = @CurrentIndexName, @IndexType = @CurrentIndexType, @PartitionNumber = @CurrentPartitionNumber, @ExtendedInfo = @CurrentExtendedInfo, @LogToTable = @LogToTable, @Execute = @Execute,@rowcount=@currentobjectrowcount,@rowcount=@currentobjectrowcount
          SET @Error = @@ERROR
        ...
        Begin
			...
			--Declare @SmallObjectScanPercent	float = 100
			--Declare @SmallObjectRows		int   = 10000000   -- 10 million
			--Declare @LargeObjectScanPercent float = 1
			--Declare @LargeObjectrows    int   = 1000000000  -- 1 billion 1,000,000,000
			--Declare @ExtraLargeObjectRowScanPercent	float = 0.0038

			if (@currentobjectrowcount <= @SmallObjectRows and @SmallObjectScanPercent >= 1)
				Begin
					Set @CurrentStatisticsSample=@SmallObjectScanPercent
					set @StatsScan=cast(@SmallObjectScanPercent as varchar(50))+' Percent'
				End

			if (@currentobjectrowcount <= @SmallObjectRows and @SmallObjectScanPercent < 1)
				Begin
					Select @CurrentStatisticsSample=ceiling((@currentobjectrowcount*@SmallObjectScanPercent)/100)
					Select @StatsScan=Cast(ceiling((@currentobjectrowcount*@SmallObjectScanPercent)/100) as varchar(50))+' Rows'
				End


			if  @currentobjectrowcount > @SmallObjectRows and @currentobjectrowcount <= @LargeObjectrows and @LargeObjectScanPercent >= 1
				Begin
					Set @CurrentStatisticsSample=@LargeObjectScanPercent
					set @StatsScan=cast(@LargeObjectScanPercent as varchar(50))+' Percent'
				End

			if  @currentobjectrowcount > @SmallObjectRows and @currentobjectrowcount <= @LargeObjectrows and @LargeObjectScanPercent < 1 
				Begin
					Select @CurrentStatisticsSample=ceiling((@currentobjectrowcount*@LargeObjectScanPercent)/100)
					Select @StatsScan=Cast(ceiling((@currentobjectrowcount*@LargeObjectScanPercent)/100) as varchar(50))+' Rows'
				End


			If @currentobjectrowcount > @LargeObjectrows and @LargeObjectScanPercent > 1
				Begin
					Set @CurrentStatisticsSample=@ExtraLargeObjectRowScanPercent
					set  @StatsScan=cast(@ExtraLargeObjectRowScanPercent as varchar(50))+' Percent'
				End
			
			if @currentobjectrowcount > @LargeObjectrows and @LargeObjectScanPercent < 1
				Begin
					Select @CurrentStatisticsSample=ceiling((@currentobjectrowcount*@ExtraLargeObjectRowScanPercent)/100)
					Select @StatsScan=Cast(ceiling((@currentobjectrowcount*@ExtraLargeObjectRowScanPercent)/100) as varchar(50))+' Rows'
				end
          ...
          IF @StatsScan IS NOT NULL OR @CurrentStatisticsResample = 'Y' OR @CurrentNoRecompute = 1 SET @CurrentCommand14 = @CurrentCommand14 + ' WITH'

          
			SET @CurrentCommand14=@CurrentCommand14 + ' SAMPLE ' +@StatsScan
			
/*
          IF (@CurrentStatisticsSample IS NOT NULL 
			SET @CurrentCommand14 =  @CurrentCommand14 + ' SAMPLE ' + CAST(@CurrentStatisticsSample AS nvarchar) + ' PERCENT'  

		  IF (@CurrentStatisticsSample IS NOT NULL AND @CurrentStatisticsSample < 1  )
			SET @CurrentCommand14 = @CurrentCommand14 + ' SAMPLE ' + CAST(@CurrentStatisticsSample AS nvarchar) + ' Rows'
			*/
		...
		Set @CurrentCommand16='
							...
							(
								SELECT 
									...
									last_user_update
								FROM
									 ['+@CurrentDatabaseName+'].sys.dm_db_index_usage_stats
								WHERE
									database_id = DB_ID('''+@CurrentDatabaseName+''')
									and [Object_ID]=@paramCurrentObjectID
							)
							SELECT @ParamQueueID =
								case when datediff(dd,MAX(last_read),Getdate()) <= @ParamHighPriTableAccessThreshold or  datediff(dd,MAX(last_write),Getdate())  <= @ParamHighPriTableAccessThreshold then 1
								when  datediff(dd,MAX(last_read),Getdate()) <= @ParamMediumPriTableAccessThreshold or  datediff(dd,MAX(last_write),Getdate())  <= @ParamMediumPriTableAccessThreshold then 2
								else 3 end 
							...
							'
		 Exec sp_executesql @statement=@currentcommand16, @params =N'@paramCurrentObjectID int,@ParamHighPriTableAccessThreshold smallint,@ParamMediumPriTableAccessThreshold smallint, @ParamQueueID tinyint output',@paramCurrentObjectID=@CurrentObjectID,@ParamHighPriTableAccessThreshold = @HighPriTableAccessThreshold, 
		 @ParamMediumPriTableAccessThreshold = @MediumPriTableAccessThreshold,@ParamQueueID=@QueueID output

          EXECUTE @CurrentCommandOutput14 = [dbo].[CommandExecute] @Command = @CurrentCommand14, @CommandType = @CurrentCommandType14, @Mode = 2, @DatabaseName = @CurrentDatabaseName, @SchemaName = @CurrentSchemaName, @ObjectName = @CurrentObjectName, @ObjectType = @CurrentObjectType, @IndexName = @CurrentIndexName, @IndexType = @CurrentIndexType, @StatisticsName = @CurrentStatisticsName ,@QueueID=@QueueID,@rowcount=@currentobjectrowcount, @LogToTable = @LogToTable, @Execute = @Execute
          SET @Error = @@ERROR
...
If  exists (select * from sys.procedures where name='Usp_UpdateStatsParallel')
Begin
	Drop Procedure  [dbo].[Usp_UpdateStatsParallel]
end
Go

Create Procedure [dbo].[Usp_UpdateStatsParallel] 
as
while exists(Select top 1 1 from CommandLog where [status] is null )
Begin
	...
	Begin Try
		Begin transaction
			...
			Update commandlog WITH (updlock, readpast)
			set [Status]='I'
			output inserted.id into @id
			from (
					Select top 1 ID from commandlog c 
					where [Status] is null
					order by QueueID
				)a
			where commandlog.id=a.id
		...
		commit transaction
			...
			Select @SQL=command from commandlog
			where id=(select * from @id)
	...
	Begin catch
		...
		Select @errormessage,@errornumber
...
GO
" -ServerInstance $TargetServerInstance -Database $Database
}

Function GenerateUpdateStatsCommand{
param (
        [Parameter(Mandatory=$True)][string]$TargetServerInstance,
        [Parameter(Mandatory=$True)][string]$Database,
        [Parameter(Mandatory=$True)][float] $SmallObjectRowScanPercent,
        [Parameter(Mandatory=$True)][long]$SmallObjectRows,
        [Parameter(Mandatory=$True)][float]$LargeObjectScanPercent,
        [Parameter(Mandatory=$True)][long]$LargeObjectrows,
        [Parameter(Mandatory=$True)][float]$ExtraLargeObjectRowScanPercent,
        [Parameter(Mandatory=$True)][int]$HighPriTableAccessThresholdDays,
        [Parameter(Mandatory=$True)][int]$MediumPriTableAccessThresholdDays
       )
#### Check for inputs
...
if($Database -eq $null -or $Database -eq '') {
    ...
    #### Deploy UpdateStats Procs
    DeployUpdateStatsSQL -TargetServerInstance $TargetServerInstance -Database $Database


    ### Generate UpdateStats Command
    Invoke-Sqlcmd -query "
    ...
    @LogToTable = 'Y',
    @Execute  = 'N',
    @SmallObjectScanPercent	 = $SmallObjectRowScanPercent,
    @SmallObjectRows		   = $SmallObjectRows,   -- 10 million
    @LargeObjectScanPercent  = $LargeObjectScanPercent,
    @LargeObjectrows       = $LargeObjectrows,  -- 1 billion 1,000,000,000
    @ExtraLargeObjectRowScanPercent	 = $ExtraLargeObjectRowScanPercent,
    @HighPriTableAccessThreshold =$HighPriTableAccessThresholdDays,
    @MediumPriTableAccessThreshold  = $MediumPriTableAccessThresholdDays

    " -ServerInstance $TargetServerInstance -Database $Database -QueryTimeout 65000
}
...
##### Import the run space module, ths is required to run parallel jobs
...
$code = {
    ...
    # code goes here
    Invoke-Sqlcmd -Query "Exec Usp_UpdateStatsParallel" -ServerInstance $TargetServerInstance -Database $Database -QueryTimeout 65000

}

#### Create Run space jobs
for($i=0;$i -lt $Threadcount ;$i++){
    New-RunspaceJob -RunspacePool $thisRunspacePool -JobName "UpdateStats_$TargetServerInstance_$Database" -ScriptBlock $code -Parameters @{TargetServerInstance = "$TargetServerInstance";Database = "$Database" }
...
RunUpdateStats -TargetServerInstance "$Instance" -Database $Database -threadcount 16;

$outputMSG = "UpdateStats jobs started....."
$outputMSG
WHILE( (show-runspacejob | where {$_.jobName -eq "UpdateStats_$Database" -and $_.status -eq 'Running' } | measure-object | Select-Object -expand count ) -gt 0 )
{
	Start-Sleep -s 10
}

IF( (show-runspacejob | where {$_.jobName -eq "UpdateStats_$Database" -and $_.hadErrors -eq 'False' } | measure-object | Select-Object -expand count ) -eq 0 )
	{
		$outputMSG = "*** SUCCESS *** All Jobs Completed with no errors."
		$outputMSG
	}
ELSE
	{
		$outputMSG = "*** FAILURE *** All Jobs Completed with no errors."
		$outputMSG
	}

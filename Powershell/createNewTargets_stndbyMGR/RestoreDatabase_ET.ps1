...
Param(
    ...
    [string]   $RestoreLunPath = $null, ## if $null, Process will try to find out the ET DB location root LUN from sys.sysaltfiles 
...
function Attach-Database( [string] $Databasenew, [string] $AttachlunPath, [string] $AttachlunPath_rem  )
{
...
# do not use write-output or host and print any value inside this function whcih will be passed backed to the caller
    ...
    Param( [string] $ServerName, [string] $FilePhysicalName, [string] $Filelogicalname, [String] $FileDatabaseName, [String] $FileOverWriteDatabase, [bool] $isDatabaseExist )
...
{
    param( [string] $Databasenew, [string] $BackupPhysicalFileName, [string] $BackupTypeDescription, [string] $HasBackupChecksums, [int] $f_dryRun )

    $BackupFile_New = $BackupPhysicalFileName
    
    Writeout -printmsg "Database Name Validation is starting for [$Databasenew] - DryRun: $f_dryRun"
    ...
    IF( $f_dryRun -eq 1)
        {
            WRITE-HOST "[SKIP] Skipping existence check - not performing restore."
        }
    ELSE
        {
    ...
    foreach ($datafilelist in $DataFileList_From_Backup)
        { 
            ...
            $DataPhysicalFileName = ($datafilelist.PhysicalName).split("\")[-1]
            $DataPhysicalFileLoc= ($datafilelist.PhysicalName).split("\")[-2]
            #Added by Stefka, 2/8/2018; modify target folder name to Data01 in case it is showing ConfigDB
            IF ( $DataPhysicalFileLoc -eq 'ConfigDB' ) { $DataPhysicalFileLoc = 'Data01' }
            #end Add
            $DatFileNewFolderPath_new = "$lunpath$DataPhysicalFileLoc\$DataPhysicalFileName"
            $DatFileNewFolderPath_new = $DatFileNewFolderPath_new.Trim()
        ...
        $RestoreCmd = "RESTORE DATABASE [$Databasenew] `n FROM `n $BackupFile_New WITH $DBFiles_tmp"
        ...
        try
            {
               ...
               # Reterive restore command
                ...
                If( $f_dryRun -eq 1)
                    {
                        WRITE-HOST "[DryRun]"
                        #WRITE-HOST `t"Invoke-Sqlcmd -ServerInstance $ServerInstance -Database master -Query $RestoreCmd -Verbose -OutputSqlErrors $true -OutBuffer $true -AbortOnError -ErrorAction stop -ConnectionTimeout 0 -QueryTimeout 0" 
                    }
                ELSE
                    {
                       WRITE-HOST "[] Execute Restore "
...
# Main CODE blocks stats here *************************************************************************
    ...
    IF ( -NOT (test-path "FileSystem::$BackupFile_rem") ) {$BackupFile_rem = $BackupFile } 
    ...
    if ($OperationType -eq "Restore")
    {
        ...
        try {
                $SQL_dl = Get-ChildItem  "filesystem::$lunpath_rem" -ErrorAction stop -Exclude 'System Volume Information' | Where-Object{$_.name -match "data[0-9]"} | Select name | sort name 
                $SQL_ll = Get-ChildItem  "filesystem::$lunpath_rem" -ErrorAction Stop -Exclude 'System Volume Information' | Where-Object{$_.name -match "log[0-9]"} | Select name | sort name 
            ...
            {
                Writeout -printmsg   "[$LunPath] is not valid or does not have Data & Log Luns - $lunpath_rem"
    ...
    if ($operationtype -eq "Restore")
        {
            ...
            Writeout -printmsg "Going to read the list of backup files located in the folder [$BackupFile] for database [$Database]" 
    ...
    for ( $i=0; $i -lt $BackupDBCnt; $i++)
    {
       ...
       # Writeout -printmsg "Starting Validation and 'Restore with NORecovery' process for the database [$database]"
        ...
        foreach ($RDHr in $RestoreDBHdr)
        {
            ...
            try{                 
                  $backupFile = ValidateAndRestore_Database -Databasenew $database -BackupPhysicalFileName $BackupListFullPath_str -BackupTypeDescription $RDHr.BackupTypeDescription -HasBackupChecksums $RDHr.HasBackupChecksums -f_dryRun $dryRun
                  #WRIte-HOST "[1] $backupFile "
...
#restoredatabase -RequestNum 711 -ServerInstance "Ind1p01c084I03\I03" -Database "ConfigDB" -RestoreType "Database" -BackupFile "G:\SQL\I03\Bak2\ExactTarget86\" -OverWriteDatabase 0 -OperationType "Restore" -Verbose -RestoreLunPath "G:\SQL\I03\ConfigDB\"

<#####################################################################
Purpose:  
     This procedure performs restores a database onto the same mounts as the source but on a new instance. 
History:  
     20171010 Praveen created
     20171011 Hbrotherton - Added dry run to skip restore
     20180110 Stefka - Added logic for ConfigDB
     20180222 hbrotherton - Added more logic for dry run to skip IF DB EXISTS and OVERWRITE = 0
     YYYYMMDD USERID W-000000000 This is what I changed.
Comments:
     Anything you feel is important to share that is not the "purpose"
     This script needs to return the BAK1 path and file for the standbyDBmanager.exe
#######################################################################>

...
QA - no phases
    .\deployGitHubObjects.ps1 -targetDB "UTILITY" -currentDomain ".QA.LOCAL" -repoFolder "StoredProcedure"
...
PARAM(
    ...
    [Parameter(Mandatory=$true)]
        [string] $cmsGroup ="NA", 
		#[string] $repoRoot="D:\Harold\myGit\Releases\"+ $targetDB,
...
function process-folder ( [string] $f_sqlSourcePath, [string] $f_targetHost, [string] $f_targetDB, [string] $f_folder, [string] $f_cmsGroup , [int] $f_dryRun)
{
           ...
           # $error = ""
            ...
            WHILE( $process -ne "Succesful" -AND $failureCounter -lt $total ) # AND attempts less than file count
            {
                ...
                get-childitem -recurse -path $f_sqlSourcePath -filter $filter | % {
                    ...
                    ELSE
                        {
                            WRITE-VERBOSE "Invoke-Sqlcmd -ServerInstance $f_targetHost -Database $f_targetDB  -ErrorAction Stop -InputFile $f_sqlSourcePath\$currentFile -verbose "
                            IF($process -ne "FAILED"){ $Process = "Succesful" }
                        }
                } #end get-childItem
                #WRITE-HOST " "
...
ForEach( $folder in $folderArray )
{
    ...
    WRITE-VERBOSE ".\deployGitHubObjects.ps1 -targetDB $folder -filter $filter -repoRoot $repoRoot -repoFolder $repoFolder -phase $phase -cmsGroup $cmsGroup -dryRun $dryRun -verbose" 
    WRITE-HOST " "
        ...
        ELSE
            {
                ...
                ForEach ( $currentHost in $hostList)
                        ...
                        IF( checkStatus-Ping $currentHost $currentDomain ) 
                            {
                                ...
                                #$targetDBInfo.Count
                                IF( ($targetDBInfo.dbState -ne "RESTORING") -AND ($targetDBInfo.Count -ne 0) )
                                    {
                                        IF( $filterExtenion -eq ".sql" )
                                            {
                                                ...
                                                If( $repoFolder -eq "Auto" )
                                                    {	
                                                        ...
                                                        ELSE
                                                            {
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "Role" $cmsGroup $dryRun
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "Schema" $cmsGroup $dryRun
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "Table" $cmsGroup $dryRun
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "View" $cmsGroup $dryRun 
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "Type" $cmsGroup $dryRun	
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "Synonym" $cmsGroup $dryRun
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "UserDefinedFunction" $cmsGroup $dryRun
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "StoredProcedure" $cmsGroup $dryRun
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "Init" $cmsGroup $dryRun
                                                                process-Folder "$repoRoot\" $currentHost $targetDB "AgentJob" $cmsGroup $dryRun
                                                            }
                                                    }
                                                ELSE
                                                    {
                                                        Process-Folder "$repoRoot\" $currentHost $targetDB $repoFolder $repoVersion $dryRun
                                                    }
                                             }
                                ...
                                ELSE  # falsely reports missing DB when instance is unreachable.
                                    {
                    ...
                    $serverCounter ++
                    WRITE-HOST "[ $serverCounter of $serverCount ]"
                    WRITE-HOST " "
                    } #forEach Host
            }
...
#$ResultsTable | Sort-Object @{Expression = "OrderValue"}, @{Expression = "TargetInstance"}  | format-Table -Property targetInstance, targetDatabase, FinalResults -AutoSize  #ForEach-Object {$_.ItemArray[1] +" "+ $_.ItemArray[2]}
    ...
    ELSE
        {
            WRITE-HOST "[] Finished Deploying: $targetDB $targetHost $phase $cmsGroup "
        }

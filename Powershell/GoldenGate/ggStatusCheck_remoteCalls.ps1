...
#  \\XTINP1DBA01\GitHub\dbops_workingfolders\dbops\Harold\powerShell\ggStatusCheck.ps1 -targetDBHost "DFW1P05C055N01" -expectedStatus "RUNNING"
...
{    <####################################
     ...
     foreach ($line in $raw)
     {
          ...
          # RETURN
            ...
            If($objectStatus -eq $expectedValue) # we really expect "RUNNING"
            {
                write-output `t`t"[OK] $objectName - $objectStatus"
            }
            ELSE
            {
                # Do not believe this can occur and was originally coded for install
                # If(!$objectStatus) {$objectStatus = "NONEXISTENT" }

                IF($objectStatus -eq "STOPPED") # stopped does not always mean there is a problem.  We STOP services during failovers.
                {
                     write-output `t`t"[WARNING] $objectName - $objectStatus - raise LESS CRITICAL alert"
                }
                ELSE # this should report on "ABENDED"
                {
                     write-output `t`t"[ALERT] $objectName - $objectStatus - raise CRITICAL alert"
                }
